...
bitstream = 'mb4k.bof.gz'
bitstream = 'mb1k-v159.bof.gz'
...
if __name__ == '__main__':
	...
	try:
		...
		logger.setLevel(20)  # debug log triggers bytes -> str decoding bug in katcp
		...
		init_10gbe('xgbe0', '192.168.16.221', 33333, '192.168.16.1', 12001)
		init_10gbe('xgbe1', '192.168.16.222', 33333, '192.168.16.2', 12002)
		init_10gbe('xgbe2', '192.168.16.223', 33333, '192.168.16.3', 12003)
		init_10gbe('xgbe3', '192.168.16.224', 33333, '192.168.16.4', 12004)
		...
		win = pg.GraphicsLayoutWidget(title='Multi-beam', show=True)
		win.resize(1280, 800)
		plts = []
		lines = []
		for u in range(0, 2):
			...
			for i in range(0, 2):
				...
				#lines.append(plt.plot(stepMode=False, fillLevel=0, brush=(0,255,0,150)))
		win.nextRow()
		for u in range(0, 2):
			unit = 'u%d' % u
			for i in range(2, 4):
